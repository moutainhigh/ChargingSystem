 ...
 */
@Service
public class PCStatisticalReportServiceImpl implements PCStatisticalReportService {
    ...
    @Override
    public DataReuslt queryPCReport(ReportReqDTO reportReqDTO) {
        Map map=new HashMap();
        List<ReportReqDTO> reportReqDTOList = reportMapper.queryPCReport(reportReqDTO);
        List<PayTypeReqDTO> payTypeList = reportMapper.getPayPCTypeList(reportReqDTO);
